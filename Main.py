...
def CompleteOrder():
    ...
    if request.method == "POST":
        if predicted_price != 0:
            ...
            student_sql_query = (
                "INSERT INTO purchaseorder(username,product_id,product_name,amount,transaction_date) VALUES('"
                + uname
                + "','"
                + product_id
                + "','"
...
def PostReviewAction():
    if request.method == "POST":
        ...
        db_cursor = db_connection.cursor()
        student_sql_query = (
            "INSERT INTO reviews(username,review,sentiment) VALUES('"
            + uname
            + "','"
            + review
            + "','"
...
def ChatData():
    if request.method == "GET":
        ...
        if (
            "final" in query.lower()
            or "discount" in query.lower()
            or "my" in query.lower()
        ):
            discount = (predicted_price / 100) * 5
            predicted_price = predicted_price - discount
            output = "The final price you can get this product is $:" + str(
                predicted_price
            )
...
def record():
    if request.method == "POST":
        ...
        res = subprocess.check_output(
            path + "ffmpeg.exe -i " + path + "audio.wav " + path + "audio1.wav",
            shell=True,
        )
...
def Chatbot():
    if request.method == "GET":
        ...
        product_id = request.args.get(
            "t1"
        )  # user will select product for which he want negotiate
        ...
        products = dataset.loc[
            dataset["index"] == product_id
        ]  # read all rows from dataset which is matches with user selected product
        ...
        output = (
            "Hi! this is Nego.<br/>Your selected Product : "
            + product_name
            + ".<br/>Its Current Price : "
            + str(original_price)
            + ".<br/>"
        )
        page = "Chatbot.html"
    ...
    if request.method == "GET":
        ...
        for i in range(len(dataset)):
            ...
            output += (
                ...
                + index
                + '.png" width="150" height="150"></img></td>'
            )
            output += "<td>" + font + price + "</font></td>"
            output += (
                '<td><a href="Chatbot?t1='
                + index
                + '&t2=text">Text Based Chatbot to Negotiate</a></td>'
            )
            output += (
                '<td><a href="Chatbot?t1='
                + index
                + '&t2=voice">Voice Based Chatbot to Negotiate</a></td></tr>'
            )
...
def SignupAction():
    if request.method == "POST":
        ...
        if status == "none":
            ...
            db_cursor = db_connection.cursor()
            student_sql_query = (
                "INSERT INTO users(username,password,contact_no,emailid,address,gender) VALUES('"
                + user
                + "','"
                + password
                + "','"
